...
const int loginTime = 30000; //30000;
...
String nameMachine = readWord(indexEeprom.indexLocation);
 ...
 //client.print(String("GET /getTcpData.htm?dayCont="+machine.contor+"&signalLevel="+String(WiFi.RSSI())+"&serialnr="+nameMachine+"&macAddress="
  //+machine.macAdress+"&permanentCont="+String(totalCoin)+" HTTP/1.1\r\n") +
   //           "Host: " + serverAddress + "\r\n" +
   //           "Connection: close\r\n\r\n");
client.print(String("GET /LastWRZone/getTcpData.htm?dayCont="+String(machine.contor)+"&signalLevel="+String(WiFi.RSSI())+"&serialnr="+nameMachine+"&macAddress="+machine.macAdress+"&permanentCont="+String(totalCoin)+" HTTP/1.1\r\n") +
                 "Host: " + serverAddress + "\r\n" +
                 "Connection: close\r\n\r\n");
...
if((timeContor > intervalTime)&&(coinPin.numberKeyPresses> 0)){
  ...
  if (client.connect(serverAddress, serverPort)) {
   ...
   // totalCoin = EEPROMRead(indexEeprom.indexTotalCoin);
    ...
    //machine.SerialMachine = "Stei1";  
   String nameMachine = readWord(indexEeprom.indexLocation);
 client.print(String("GET /LastWRZone/getDataHall.htm?dayCont="+machine.contor+"&macAddress="+String(WiFi.macAddress())+"&permanentCont="+String(totalCoin)+" HTTP/1.1\r\n") +
                 "Host: " + serverAddress + "\r\n" +
                 "Connection: close\r\n\r\n");
